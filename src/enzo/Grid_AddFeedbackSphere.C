...
//            Convert back to number density
              ...
              //NOTE: THE PROBLEM IS NOT HERE!
              //SNum, SplusNum,
              //      SiNum, SiplusNum, CHplusNum, CH2plusNum, SiONum, H3plusNum;
              //
              if (water_rates == 3){
                BaryonField[SNum][index]       *= increase;
                BaryonField[SplusNum][index]   *= increase;
                BaryonField[SiNum][index]      *= increase;
                BaryonField[SiplusNum][index]  *= increase;
                BaryonField[CHplusNum][index]  *= increase;
                BaryonField[CH2plusNum][index] *= increase;
                BaryonField[SiONum][index]     *= increase;
